  ...
  try {
    ...
    if (superAdminRole.length === 0) {
      ...
      if (existingSuper.length > 0) {
        return NextResponse.json({ error: 'Super admin already set up. Please sign in.' }, { status: 400 });
      }
    }
    ...
    const message = error instanceof Error ? error.message : 'Database error during setup';
